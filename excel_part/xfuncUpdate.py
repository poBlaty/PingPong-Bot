    ...
    for i in range(len(value)):
        ...
        for j in range(1, len(sheet['Unnamed: 0'])):
            if sheet['Unnamed: 0'].values[j] in matches:
                ...
                players.append([sheet['День/Время/Стол'].values[j], sheet['Unnamed: 24'].values[j], sheet['Unnamed: 25'].values[j], sheet['День/Время/Стол'].values[j+1]])
                score.append(f"{scoresheet['Unnamed: 10']}, {scoresheet['с какого места начинается сетка ↓']}, {scoresheet['Unnamed: 12']}, {scoresheet['Unnamed: 13']}, {scoresheet['Unnamed: 14']}, {scoresheet['Unnamed: 15']}, {scoresheet['Unnamed: 16']}, {scoresheet['Unnamed: 17']}, {scoresheet['Unnamed: 18']}, {scoresheet['Unnamed: 19']}, {scoresheet['2']}, {scoresheet['Фамилия Имя']}, {scoresheet['Unnamed: 22']}, {scoresheet['Unnamed: 23']}")
        ...
        for j in range(1, len(matches)):
            if matches[j][2] == 'x' or matches[j][2] == 'х' or matches[j][-1] == 'x' or matches[j][-1] == 'х' or int(matches[j][3])+int(matches[j][4])<3:
                continue
            else:
