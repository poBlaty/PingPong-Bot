...
def LastComp(name: str) -> list:  # Результаты встреч на последних сыгранных соревах
    ...
    for i in range(l - 1, -1, -1):
        ...
        if (str(df_list_match["Имя 1"].values[i]).lower().title() == name or str(
                df_list_match["Имя 2"].values[i]).lower().title() == name) and df_list_match["Название соревнований"].values[i] == lastComp:
            result.append([str(df_list_match["Имя 1"].values[i]).lower().title(), df_list_match["Общий счет"].values[i],
                           str(df_list_match["Имя 2"].values[i]).lower().title(), point])
...
def BestWins(name: str, value: int) -> list:  # Лучшие победы
    ...
    for i in range(l - 1, -1, -1):
        ...
        if (str(df_list_match["Имя 1"].values[i]).lower().title() == name or str(
                df_list_match["Имя 2"].values[i]).lower().title() == name) and winner == name:
            result.append([df_list_match["Название соревнований"].values[i],
                           str(df_list_match["Имя 1"].values[i]).lower().title(),
                           int(df_list_match["Рейтинг 1"].values[i]), df_list_match["Общий счет"].values[i],
                           str(df_list_match["Имя 2"].values[i]).lower().title(),
                           int(df_list_match["Рейтинг 2"].values[i]), point])
...
def HeadToHead(fullname1: str, fullname2: str) -> list:  # Статистика личных встреч между двумя игроками
    ...
    for i in range(l - 1, -1, -1):
        ...
        if (str(df_list_match["Имя 1"].values[i]).lower().title() in fullname and str(
                df_list_match["Имя 2"].values[i]).lower().title() in fullname) and winner:
            result.append([df_list_match["Название соревнований"].values[i],
                           str(df_list_match["Имя 1"].values[i]).lower().title(),
                           int(df_list_match["Рейтинг 1"].values[i]), df_list_match["Общий счет"].values[i],
                           str(df_list_match["Имя 2"].values[i]).lower().title(),
...
def CompsValueYear(year: int, name: str) -> int:  # Количество сорев (На вход: (год, имя)) если без них, то 0 и ''
    ...
    if year != 0 and name != '':
        for i in range(l - 1, -1, -1):
            if (str(df_list_match['Имя 1'].values[i]).lower().title() == name or str(
                    df_list_match['Имя 2'].values[i]).lower().title() == name) and int(
                    str(df_list_match["Дата"].values[i]).split(' ')[-2]) == year:
                result.append(df_list_match["Название соревнований"].values[i])
...
def SetsValueYear(year: int, name: str) -> int:  # Количество партий (На вход: (год, имя)) если без них, то 0 и ''
    ...
    if year == 0 and name == '':
        for i in range(l - 1, -1, -1):
            sum += int(str(df_list_match["Общий счет"].values[i]).split(':')[0]) + int(
                str(df_list_match["Общий счет"].values[i]).split(':')[1])
...
def WinMatchValueYear(year: int,
                      name: str) -> int:  # Количество выигранных матчей (На вход: (год, имя)) если без года, то 0
    df_list_match = pd.read_excel("data/Список матчей.xlsx")
    l = len(df_list_match["Имя 1"])
...
def WinSetsValueYear(year: int,
                     name: str) -> int:  # Количество выигранных партий (На вход: (год, имя)) если без года, то 0
    df_list_match = pd.read_excel("data/Список матчей.xlsx")
    l = len(df_list_match["Имя 1"])
    sum = 0
...
def WinPointsValueYear(year: int,
                       name: str) -> int:  # Количество выигранных очков (На вход: (год, имя)) если без года, то 0
    df_list_match = pd.read_excel("data/Список матчей.xlsx")
    l = len(df_list_match["Имя 1"])
    sets = ['Партия 1 1', 'Партия 1 2', 'Партия 2 1', 'Партия 2 2', 'Партия 3 1', 'Партия 3 2', 'Партия 4 1',
            'Партия 4 2', 'Партия 5 1', 'Партия 5 2', 'Партия 6 1', 'Партия 6 2', 'Партия 7 1', 'Партия 7 2']
        ...
        for i in range(l):
            if f"{df_raiting['Фамилия'].values[i]} {df_raiting['Имя'].values[i]}" == name:
                for j in range(len(col) - 2):
                    result.append([col[j + 2], str(df_raiting[col[j + 2]].values[i]).split(', ')])

    else:
        for i in range(l):
            if f"{df_raiting['Фамилия'].values[i]} {df_raiting['Имя'].values[i]}" == name:
                for j in range(1, 13):
                    result.append([col[-1 * j], str(df_raiting[col[-1 * j]].values[i]).split(', ')])
